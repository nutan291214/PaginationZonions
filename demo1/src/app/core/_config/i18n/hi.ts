// Hindi
export const locale = {
	lang: 'hi',
	data: {
		TRANSLATOR: {
			SELECT: 'अपनी भाषा का चयन करें',
		},
		MENU: {
			DASHBOARD: 'डैशबोर्ड',
			Restaurants: 'रेस्टोरेंट',
			Create_Restaurant: 'रेस्तरां बनाएँ',
			Restaurant_list: 'रेस्तरां सूची',
			Active_Restaurant: 'सक्रिय रेस्तरां',
			User_Management: 'उपयोगकर्ता प्रबंधन',
			Active_Users: 'सक्रिय उपयोगकर्ता',
			DETAILS: 'तपशील',
				},
		RESTAURANTLIST: {
			ACTIVE: 'सक्रिय रेस्तरां'
		},
		AUTH: {
			GENERAL: {
				OR: 'या',
				SUBMIT_BUTTON: 'प्रस्तुत',
				NO_ACCOUNT: 'खाता नहीं है?',
				SIGNUP_BUTTON: 'साइन अप करें',
				FORGOT_BUTTON: 'पासवर्ड भूल गए',
				BACK_BUTTON: 'वापस',
				PRIVACY: 'Privacy',
				LEGAL: 'Legal',
				CONTACT: 'Contact',
			},
			LOGIN: {
				TITLE: 'लॉगिन खाता',
				BUTTON: 'साइन इन करें',
				LOGINBUTTON: 'लॉगिन'
			},
			FORGOT: {
				TITLE: 'पासवर्ड भूल गए?',
				DESC: 'अपना पासवर्ड रीसेट करने के लिए अपना ईमेल प्रविष्ट करें',
				SUCCESS: 'आपका खाता सफलतापूर्वक रीसेट कर दिया गया है।'
			},
			REGISTER: {
				TITLE: 'साइन अप करें',
				DESC: 'अपना खाता बनाने के लिए अपना विवरण दर्ज करें',
				SUCCESS: 'आपका खाता सफल पंजीकृत हो गया है।'
			},
			INPUT: {
				EMAIL: 'ईमेल',
				FULLNAME: 'पूरा नाम',
				PASSWORD: 'कुंजिका',
				CONFIRM_PASSWORD: 'पासवर्ड की पुष्टि कीजिये',
				USERNAME: 'उपयोगकर्ता नाम'
			},
			VALIDATION: {
				INVALID: '{{name}} मान्य नहीं है',
				REQUIRED: '{{name}} आवश्यक है',
				MIN_LENGTH: '{{name}} न्यूनतम लंबाई है {{min}}',
				AGREEMENT_REQUIRED: 'नियम और शर्तों को स्वीकार करना आवश्यक है',
				NOT_FOUND: 'अनुरोध किया {{name}} नहीं मिला',
				INVALID_LOGIN: 'लॉगिन विवरण गलत है',
				REQUIRED_FIELD: 'आवश्यक क्षेत्र',
				MIN_LENGTH_FIELD: 'न्यूनतम क्षेत्र की लंबाई:',
				MAX_LENGTH_FIELD: 'अधिकतम क्षेत्र की लंबाई:',
				INVALID_FIELD: 'फ़ील्ड मान्य नहीं है',
			}
		},
	}
};
